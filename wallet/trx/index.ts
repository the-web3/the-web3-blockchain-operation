...
function signTx (privateKey, transaction) {
  const signedTx = cryptoSignTransaction(privateKey, transaction);
  return {
    signedTx,
    hex: signedTx.hex,
...
function createTRC20Tx (params: any) {
  ...
  const toAddressHex: string = decode58ToHex(to);
...
function createTransaction (params) {
  ...
  const transferContractTx = buildTransferContract(
    transferContract,
...
function triggerSmartContract (params) {
  ...
  const smartContractTx = buildTransferContract(
    contract,
...
function triggerAssetContract (params) {
  const assetContract = new TransferAssetContract();
  ...
  const assetContractTx = buildTransferContract(
    assetContract,
